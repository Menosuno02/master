...
--	Procedimiento para los empleados un número de departamento.
--	Necesitamos también la suma salarial de dichos empleados por departamento
CREATE OR ALTER PROCEDURE SP_EMPLEADOS_DEPARTAMENTO_OUT
(@IDDEPT INT, @SUMASALARIO INT OUT)
AS
	SELECT *
	FROM EMP
	WHERE DEPT_NO = @IDDEPT
	--	Necesitamos asignar valores a los parámetros de salida dentro del procedimiento
	SELECT @SUMASALARIO = SUM(SALARIO) 
	FROM EMP
	WHERE DEPT_NO = @IDDEPT
GO

--	Para la petición, necesitamos una variable para almacenar el parámetro de salida
DECLARE @RESPUESTA INT
EXECUTE SP_EMPLEADOS_DEPARTAMENTO_OUT 20, @RESPUESTA OUTPUT
PRINT 'SUMA SALARIAL = ' + CAST(@RESPUESTA AS NVARCHAR)
...
SELECT APELLIDO, SALARIO, HOSPITAL_COD
FROM DOCTOR) AS QUERY
WHERE QUERY.HOSPITAL_COD = 17;

SELECT ISNULL(SUM(SALARIO), 0), ISNULL(AVG(SALARIO), 0), COUNT(APELLIDO) FROM
	(SELECT APELLIDO, SALARIO, HOSPITAL_COD
	FROM PLANTILLA
	UNION
	SELECT APELLIDO, SALARIO, HOSPITAL_COD
